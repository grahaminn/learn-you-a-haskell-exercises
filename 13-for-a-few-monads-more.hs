{-
 ...
 -   
 - Note, There appear to have been some changes to ghc since Learn You a Haskell was written. 
 - There is no data constructor exposed for Writer, like described in the chapter. 
 - You cannot do things like: logNumber x = Writer (x, ["Got number: " ++ show x]). However there is a 'writer' function, which works in the same way as that constructor would. 
 - That is, you could do: logNumber x = writer (x, ["Got number: " ++ show x]). For more information, see http://stackoverflow.com/questions/11684321/how-to-play-with-control-monad-writer-in-haskell
